...
- **自動判定(既定)** — 行末コメント記号(`//` `#` `/*`)を持つ行が過半なら「コメント揃え」、それ以外は「表揃え」。判定結果を画面に表示します(自動判定では「全部揃え」「関数・括弧揃え」は選ばれません)。
...
- **関数・括弧揃え** — 区切り欄ではなく行の構造(`(` `)` `;`)そのものをアンカーにして揃えます(下の例J)。関数名の長さが違っても開き括弧・閉じ括弧・セミコロンの桁が揃います。区切り欄と `=`揃えは使いません(画面上でもグレーアウトします)。
...
関数名の長さが違っても `(` `)` `;` が同一桁に揃います。コード部の連結はギャップ0固定(閉じ括弧やセミコロンの手前に余計な空白を入れないため)で、ギャップ設定は行末コメントの開始位置にだけ効きます。行末にブロックコメントが付いていれば、`/*` と `*/` の桁も同時に揃います。

アンカーの決め方は「先頭から数えた `(`」「末尾から数えた `)`」「末尾の `;`」で、**対象行すべてに存在するものだけ**を採用します。たとえば `foo(bar(x));` と `baz(y);` が混在する場合、2番目の `(` は片方にしかないので採用されず、`(` `)` `;` の3か所で揃います。引用符の中の `(` `)` `;` はアンカーになりません。

## マクロ(プリプロセッサ指令)の扱い

`#define` の定数表や `#include` の並びも、そのまま貼れば揃います。
...
`#` は本来コメント記号(シェルや Python など)として扱いますが、**行頭の `#` が C/C++ のプリプロセッサ指令**(`#define` `#include` `#undef` `#ifdef` `#ifndef` `#if` `#elif` `#else` `#endif` `#line` `#error` `#warning` `#pragma`)のときだけはコードとして扱います。`# これはコメント` や `x = 1 # メモ` は従来どおりコメントです。

## 行頭インデントを揃える(v1.4)

オプション列のチェックボックス「**行頭インデントを揃える**」をONにすると、整形の前に**行頭の空白幅を揃えます**。貼り付け元で行頭がバラバラになっている塊を、エディタで下ごしらえせずにそのまま流せます。
...
- 合わせる先は**いちばん多いインデント幅**です。同数のときは小さいほう(余計に字下げしない)を採ります
- 空行はそのまま残ります。タブと半角スペースが混ざっていても、タブ幅で展開して判定します
- **既定はオフ**です。`typedef struct {` と中のメンバーのように、意図的に字下げが違う入力を勝手に潰さないためです
...
- 行頭タブの扱いは入力全体を見て決まります。**非空行がすべて行頭タブで始まる**場合(ソースコードの共通インデント)、および**タブを含まない行が混ざっている**場合(構造体を `typedef struct {` の行ごとコピーしたときなど)は、タブ幅で空白展開してインデントとして保持します。**すべての行がタブを含み、一部の行だけが行頭タブ**の場合(Excel 貼り付けで先頭列が空のケース)は、区切りに `\t` が含まれるとき先頭に空セルができ、列が正しく揃います。`\t` を欄から消した場合は行頭タブは常にインデント扱いです。
